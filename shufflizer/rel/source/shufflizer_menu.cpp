...
void ShufflizerMenu::Draw() {
    ...
    if (menu_page_ == 1) {
        color = (menu_state_ == MenuState::HP_MODIFIER 
                 ? -0xFFFFU : -0xFFU) | alpha;
        sprintf(buf, "HP modifier: %d%s", options_->enemy_hp_modifier, "%");
        DrawString(buf, -205, -73, color, 0.75);
        
        color = (menu_state_ == MenuState::ATK_MODIFIER
                 ? -0xFFFFU : -0xFFU) | alpha;
        sprintf(buf, "ATK modifier: %d%s", options_->enemy_atk_modifier, "%");
        DrawString(buf, -205, -92, color, 0.75);
        
        color = (menu_state_ == MenuState::DEF_MODIFIER
                 ? -0xFFFFU : -0xFFU) | alpha;
        sprintf(buf, "DEF modifier: +%d", options_->enemy_def_modifier);
    ...
    sprintf(buf, "Next Page (%d of 2)", menu_page_);
